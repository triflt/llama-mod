...
class MoDLlamaTrainer():
    ...
    def compute_causal_loss(self, weights, topk_indices):
        ...
        return criterion(token_weights.flatten(), aux_targets), aux_targets
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            for i, (inputs, targets) in enumerate(tqdm(self.dataloader, desc=f"Epoch: {epoch+1}")):
                ...
                outputs = self.model(inputs.to(device), start_pos=0)

                loss = criterion(outputs['output'].permute(0, 2, 1), targets.to(device))
